...
namespace VasilevTools.Commands
{
    public sealed class SwPropertyNCommand : ISwCommand
    {
        ...
        private static void RunProcessing(ISldWorks swApp, IModelDoc2 startModel, Settings settings, string settingsPath)
        {
            ...
            var progressOpenedAt = DateTime.UtcNow;
            ...
            {
                progress.Show();
                progress.UpdateProgress(0, 100, "Сбор структуры сборки...");
                ...
                var shownFor = (int)(DateTime.UtcNow - progressOpenedAt).TotalMilliseconds;
                if (shownFor < minVisibleMs)
                {
                    PauseWithDoEvents(minVisibleMs - shownFor);
                }
        ...
        private sealed class Settings
        {
            ...
            public static Settings Load(string path)
            {
                ...
                {
                    s.ExceptionsText = string.Join(System.Environment.NewLine, exceptionItems.Where(x => !string.IsNullOrWhiteSpace(x)).Select(x => x.Trim()));
                }
