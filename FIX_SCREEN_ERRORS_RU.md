...
## Ошибки компиляции: `Environment` неоднозначен и `SwAddin.control` не используется

1. Если видите `Environment является неоднозначной ссылкой ...`:
   - используйте полное имя `System.Environment` в коде (например, `System.Environment.NewLine`).

2. Если видите предупреждение `Поле "SwAddin.control" никогда не используется`:
   - удалите поле `control` из `SwAddin`, **или** реально используйте его (например, присвойте и вызовите `Initialize(...)`).
   - это предупреждение не ломает сборку, но лучше убрать, чтобы не путаться.


Дополнительно по прогресс-форме:
- теперь она открывается до сбора структуры (до первого файла),
- держится минимум ~1.8 сек даже на очень быстрых кейсах,
- и остаётся TopMost во время выполнения.
